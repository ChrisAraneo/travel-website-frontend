...
class InputPhotos extends React.Component {
    ...
    add(name, base64) {
        ...
        this.setState({ photos: [...photos, { name, base64 }] }, () => this.validate(this.state.photos));
    }

    remove(name) {
        this.setState({ photos: this.state.photos.filter(item => (item.name !== name)) }, () => this.validate(this.state.photos));
    }
                            ...
                            <ul className="list" style={{ boxSizing: 'border-box', border: '1px solid #48c774' }}>
                                {this.state.photos.map(item => (
                                    <li key={item.name} className="list-item" style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                        {item.name}
                                        <a className="delete is-small" style={{ display: 'block', lineHeight: '18px' }} onClick={() => this.remove(item.name)}></a>
                                    </li>
                                ))}
                            </ul>
                        </div>
