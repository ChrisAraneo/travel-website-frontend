...
class InputList extends React.Component {
    ...
    add(item) {
        ...
        this.setState({ selected: [...selected, item] }, () => this.validate(this.state.selected));
    }

    remove(value) {
        this.setState({ selected: this.state.selected.filter(item => (item[this.props.value] !== value)) }, () => this.validate(this.state.selected));
    }
                        ...
                        <div style={{ marginBottom: '.75em' }}>
                            <ul className="list" style={{ boxSizing: 'border-box', border: '1px solid #48c774' }}>
                                {this.state.selected.map(item => (
                                    <li key={item[this.props.value]} className="list-item" style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                        {this.props.name(item)}
                                        <a className="delete is-small" style={{ display: 'block', lineHeight: '18px' }} onClick={() => this.remove(item[this.props.value])}></a>
                                    </li>
                                ))}
                            </ul>
                        </div>
